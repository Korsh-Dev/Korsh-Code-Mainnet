...
class WalletTest(BitcoinTestFramework):
    ...
    def run_test(self):
        ...
        # Send 210 KSH from 0 to 2 using sendtoaddress call.
        ...
        # node0 should end up with 1000 KSH in block rewards plus fees, but
        ...
        # Send 100 KSH with subtract fee from amount
        ...
        # Sendmany 100 KSH with subtract fee from amount
        ...
        for target, mode in product([-1, 0], ["smt/kb", "sat/b"]):
        ...
        for cointype in range(2, 6):
            len_cointype = len(self.nodes[0].listunspent(minconf=0, maxconf=0, include_unsafe=True, query_options={'coinType': cointype}))
            cointype_counts[cointype] = len_cointype
        # Coin type expectations depend on masternode collateral value.
        # Historical Dash-based tests used 1000 as collateral.
        assert_equal(cointype_counts[3], 2)
        if MASTERNODE_COLLATERAL == 1000:
            assert_equal(cointype_counts[4], 1)
        else:
            assert_equal(cointype_counts[4], 0)
        assert_equal(cointype_counts[5], 1)
        assert cointype_counts[2] in (0, 1)
        ...
        if not self.options.descriptors:
            ...
            # This will raise an exception for attempting to get the private key of an invalid Korsh address
            assert_raises_rpc_error(-5, "Invalid Korsh address", self.nodes[0].dumpprivkey, "invalid")

            # This will raise an exception for attempting to set a label for an invalid Korsh address
            assert_raises_rpc_error(-5, "Invalid Korsh address", self.nodes[0].setlabel, "invalid address", "label")

            # This will raise an exception for importing an invalid address
            assert_raises_rpc_error(-5, "Invalid Korsh address or script", self.nodes[0].importaddress, "invalid")
            ...
            for target, mode in product([-1, 0], ["korsh/kb", "duff/b"]):
                assert_raises_rpc_error(-8, 'Invalid estimate_mode parameter, must be one of: "unset", "economical", "conservative"',
                    self.nodes[2].sendtoaddress, address=address, amount=1, conf_target=target, estimate_mode=mode)
