...
def _resolve_pow_util_path():
    global _POW_UTIL_PATH, _POW_UTIL_LOOKED_UP
    if _POW_UTIL_LOOKED_UP:
...
def _run_pow_util(command, header):
    util_path = _resolve_pow_util_path()
    if util_path is None:
        return None
    proc = subprocess.run(
        [util_path, command, header.hex()],
        capture_output=True,
        text=True,
        check=False,
    )
...
def dashhash(s):
    if s in _POW_HASH_CACHE:
        return _POW_HASH_CACHE[s]

    hash_hex = _run_pow_util("hash", s)
    if hash_hex:
        pow_hash = bytes.fromhex(hash_hex)[::-1]
    else:
        pow_hash = dash_hash.getPoWHash(s)

    _POW_HASH_CACHE[s] = pow_hash
    return pow_hash
...
class CBlock(CBlockHeader):
    ...
    def solve(self):
        header_bytes = super().serialize()
        if header_bytes in _POW_GRIND_CACHE:
            grinded_header = _POW_GRIND_CACHE[header_bytes]
        else:
            grinded_header_hex = _run_pow_util("grind", header_bytes)
            grinded_header = bytes.fromhex(grinded_header_hex) if grinded_header_hex else None
            _POW_GRIND_CACHE[header_bytes] = grinded_header

        if grinded_header is not None:
            CBlockHeader.deserialize(self, BytesIO(grinded_header))
            self.rehash()
            return
