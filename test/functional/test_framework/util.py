...
def assert_fee_amount(fee, tx_size, feerate_DASH_kvB):
    ...
    if fee < target_fee:
        raise AssertionError("Fee of %s KSH too low! (Should be %s KSH)" % (str(fee), str(target_fee)))
    # allow the wallet's estimation to be at most 2 bytes off
    high_fee = get_fee(tx_size + 2, feerate_DASH_kvB)
    if fee > high_fee:
        raise AssertionError("Fee of %s KSH too high! (Should be %s KSH)" % (str(fee), str(target_fee)))
...
def check_json_precision():
    """Make sure json library being used does not lose precision converting KSH values"""
...
def get_fee(tx_size, feerate_dash_kvb):
    """Calculate the fee in KSH given a feerate is KSH/kvB. Reflects CFeeRate::GetFee"""
    ...
    return satoshi_round(target_fee_sat / Decimal(1e8)) # Truncate KSH result to nearest sat
...
def get_auth_cookie(datadir, chain):
    user = None
    password = None
    if os.path.isfile(os.path.join(datadir, "korsh.conf")):
        with open(os.path.join(datadir, "korsh.conf"), 'r', encoding='utf8') as f:
            for line in f:
