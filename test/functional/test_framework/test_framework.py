...
def _normalize_config_path(path):
    """Translate MSYS-style absolute paths to native Windows paths."""
    if os.name != "nt":
        return path
    match = MSYS_PATH_RE.match(path)
    if match is None:
...
def _normalize_config_environment_paths(config):
    if os.name != "nt":
        return
    for key in ("SRCDIR", "BUILDDIR", "RPCAUTH"):
        if key in config["environment"]:
            config["environment"][key] = _normalize_config_path(config["environment"][key])


def _resolve_default_binary(builddir, exeext, candidates):
    """Pick the first candidate binary that exists in the build tree."""
    for candidate in candidates:
        candidate_path = os.path.join(builddir, "src", candidate + exeext)
        if os.path.isfile(candidate_path):
            return candidate_path
    return os.path.join(builddir, "src", candidates[0] + exeext)
...
class BitcoinTestFramework(metaclass=BitcoinTestMetaClass):
    ...
    def parse_args(self):
        ...
        _normalize_config_environment_paths(config)
        self.config = config
    ...
    def set_binary_paths(self):
        ...
        for attribute_name, definition in binaries.items():
            env_override = next((os.getenv(env) for env in definition["envs"] if os.getenv(env)), None)
            default_filename = _resolve_default_binary(builddir, exeext, definition["candidates"])
            setattr(self.options, attribute_name, env_override or default_filename)
    ...
    def setup(self):
        ...
        runtime_candidates = [
            os.path.join(config['environment']['BUILDDIR'], '.local', 'msys_extract', 'msys64', 'mingw64', 'bin'),
        ...
        for runtime_path in runtime_candidates:
            if os.path.isdir(runtime_path):
                extra_path_entries.append(runtime_path)
        os.environ['PATH'] = os.pathsep.join(extra_path_entries + [os.environ['PATH']])
            ...
            if os.name == "nt":
                def _onerror(func, path, exc_info):
                    ...
                    try:
                        ...
                        pass
                    func(path)
                shutil.rmtree(self.options.tmpdir, onerror=_onerror)
            else:
    ...
    def _initialize_chain(self):
        ...
        if not os.path.isdir(cache_node_dir):
            ...
            if os.name == "nt" and os.path.isdir(wallets_dir):
                # On Windows, wallet directories may be created with the read-only attribute set.
                subprocess.run(["attrib", "-R", wallets_dir], check=False, stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)
            os.rmdir(wallets_dir)  # Remove empty wallets dir
