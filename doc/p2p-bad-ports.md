When Korsh Core automatically opens outgoing P2P connections, it chooses
...
A malicious actor may gossip an address:port where no Korsh node is listening,
or one where a service is listening that is not related to the Korsh network.
As a result, this service may occasionally get connection attempts from Korsh
nodes.

"Bad" ports are ones used by services which are usually not open to the public
and usually require authentication. A connection attempt (by Korsh Core,
trying to connect because it thinks there is a Korsh node on that
...
They are also considered "bad" ports as they require clients to either run Korsh
...
Below is a list of "bad" ports which Korsh Core avoids when choosing a peer to
