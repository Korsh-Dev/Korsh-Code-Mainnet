...
---------------------------------------------------------
The MN_RR hard fork, first included in Korsh Core v20, will be activated after v21 is adopted by masternodes. This
hard fork enables the major feature included in this release: Masternode Reward Location Reallocation. The activation
will also initiate the launch of the Korsh Platform Genesis Chain.
...
The Korsh Core repository (`github.com/SmartiesCoin/Korsh`) will now use `develop` as the default branch. New clones
of the repository will no longer default to the `master` branch. If you want to continue using a stable version of
Korsh Core, you should manually checkout the `master` branch.
...
can be adjusted using the new `-maxapsfee` configuration option. (korsh#5930)

### Experimental Descriptor Wallets

Please note that Descriptor Wallets are still experimental and not all expected functionality
is available. Additionally there may be some bugs and current functions may change in the future.
Bugs and missing functionality can be reported to the [issue tracker](https://github.com/SmartiesCoin/Korsh/issues).
...
Users who already have a Korsh Core wallet can continue to use it as they did before without
any change in behavior. Newly created Legacy Wallets (which is the default type of wallet) will
behave as they did in previous versions of Korsh Core.
...
- The `-banscore` configuration option, which modified the default threshold for
  ...
  changes in this release to the handling of misbehaving peers. (korsh#5511)
...
- A new `listdescriptors` RPC is available to inspect the contents of descriptor-enabled wallets.
  ...
  For ranged descriptors, it also returns the range boundaries and the next index to generate addresses from. (korsh#5911)
...
- Backwards compatibility has been dropped for two `getaddressinfo` RPC
  ...
  20.1, the `labels` field returns a JSON array of label names. (korsh#5823)
- `getnetworkinfo` now returns fields `connections_in`, `connections_out`,
  ...
  `outboundmnconnections` and `mnconnections` are removed (korsh#5823)
- `getpeerinfo` no longer returns the `banscore` field unless the configuration
  option `-deprecatedrpc=banscore` is used. The `banscore` field will be fully
  removed in the next major release. (korsh#5511)
...
Korsh Core's composite commands such as `quorum list` or `bls generate` now are compatible with a whitelist feature.
...
silently overwriting earlier parameter values with later ones. (korsh#6050)
...
- A korshd node will no longer broadcast addresses to inbound peers by default.
...
korsh-tx Changes
---------------
- When creating a hex-encoded Korsh transaction using the `korsh-tx` utility
...
- A new `-rpcwaittimeout` argument to `korsh-cli` sets the timeout
  in seconds to use with `-rpcwait`. If the timeout expires,
  `korsh-cli` will report a failure. (korsh#5923)
- The `connections` field of `korsh-cli -getinfo` is expanded to return a JSON
  ...
  returned a single integer value for the total number of peer connections. (korsh#5823)
...
- CLI `-addrinfo` now returns a single field for the number of `onion` addresses
  ...
  for Tor V2 addresses was removed from Korsh Core in 18.0.

korsh-wallet changes
-------------------
This release introduces several improvements and new features to the `korsh-wallet` tool, making it more versatile and
user-friendly for managing Korsh wallets.

- Wallets created with the `korsh-wallet` tool will now utilize the `FEATURE_LATEST` version of wallet which is the HD
...
[set-of-changes]: https://github.com/SmartiesCoin/Korsh/compare/v20.1.1...dashpay:v21.0.0
