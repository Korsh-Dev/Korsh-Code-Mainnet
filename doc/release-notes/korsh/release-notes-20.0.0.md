...
Currently, 10% of block rewards are set aside for the Korsh DAO treasury which
...
The asset lock process provides a mechanism for locking and unlocking Korsh in a
credit pool. This credit pool mirrors the available credit supply in Korsh
Platform. When a certain amount of Korsh is locked, a proportional number of
credits are issued to the identity specified by the transaction issuer. The
identity then uses the credits to pay fees for operations done on Korsh
Platform. Masternode identities receive the fees and can convert them back to
Korsh by withdrawing them from Korsh Platform.
...
### Removal of reject network messages from Korsh Core (BIP61)

This feature has been disabled by default since Korsh Core version 19.0.0.
Nodes on the network can not generally be trusted to send valid ("reject")
messages, so this should only ever be used when connected to a trusted node.

Since Korsh Core version 20.0.0 there are extra changes:
...
- It is possible to run Korsh Core as an I2P service and connect to such
...
- The `fundrawtransaction`, `sendmany`, `sendtoaddress`, and
  ...
  fee estimation methods, `KSH/kB` and `duff/B`. The target is the fee
...
- `testmempoolaccept` and `sendrawtransaction` no longer return the P2P
  ...
  Section [_Removal of reject network messages from Korsh Core (BIP61)_](#removal-of-reject-network-messages-from-korsh-core-bip61) above for
...
### Korsh-specific changes in existing RPCs:
...
- The `korsh-cli -getinfo` command now displays the wallet name and balance for
...
Please check `Help -> Command-line options` in Qt wallet or `korshd --help` for
more information.
...
python test framework and `korsh-cli` tool both take advantage of this, so
for example:

```sh
korsh-cli -named createwallet wallet_name=mywallet load_on_startup=1
```

Can now be shortened to:

```sh
korsh-cli -named createwallet mywallet load_on_startup=1
```
...
with Korsh’s product needs, such as SegWit and RBF, are excluded from our
...
[i2p]: https://github.com/SmartiesCoin/Korsh/blob/v20.x/doc/i2p.md
[guix-readme]: https://github.com/SmartiesCoin/Korsh/blob/v20.x/contrib/guix/README.md
[json-rpc]: https://github.com/SmartiesCoin/Korsh/blob/v20.x/doc/JSON-RPC-interface.md#parameter-passing
[set-of-changes]: https://github.com/SmartiesCoin/Korsh/compare/v19.3.0...dashpay:v20.0.0
