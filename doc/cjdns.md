# CJDNS support in Korsh Core

It is possible to run Korsh Core over CJDNS, an encrypted IPv6 network that
...
done outside of Korsh Core, similarly to a VPN (either in the host/OS or on
...
redundancy and robustness for both the Korsh network and individual nodes.
...
before it will work with your Korsh Core node. This is described in steps
...
## Run Korsh Core with CJDNS

Once you are connected to the CJDNS network, the following Korsh Core
...
When enabled, this option tells Korsh Core that it is running in an
...
IPv6 local network. This helps Korsh Core perform better address management:
...
CJDNS support was added to Korsh Core in version 21.0 and there may be fewer
CJDNS peers than Tor or IP ones. You can use `korsh-cli -addrinfo` to see the
...
[doc/tor.md](https://github.com/SmartiesCoin/Korsh/blob/master/doc/tor.md) for
details.

## CJDNS-related information in Korsh Core

There are several ways to see your CJDNS address in Korsh Core:
- in the "Local addresses" output of CLI `-netinfo`
- in the "localaddresses" output of RPC `getnetworkinfo`

To see which CJDNS peers your node is connected to, use `korsh-cli -netinfo 4`
or the `getpeerinfo` RPC (i.e. `korsh-cli getpeerinfo`).

You can use the `getnodeaddresses` RPC to fetch a number of CJDNS peers known to your node; run `korsh-cli help getnodeaddresses` for details.
