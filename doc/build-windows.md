...
Below are some notes on how to build Korsh Core for Windows.

The options known to work for building Korsh Core on Windows are:
...
Note that for WSL the Korsh Core source path MUST be somewhere in the default mount file system, for
example /usr/src/korsh, AND not under /mnt/d/. If this is not the case the dependency autoconf scripts will fail.
...
way. This will install to `c:\workspace\korsh`, for example:

    make install DESTDIR=/mnt/c/workspace/korsh
