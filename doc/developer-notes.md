...
- [Development guidelines](#development-guidelines)
    - [General Korsh Core](#general-korsh-core)
    - [Wallet](#wallet)
    - [General C++](#general-c)
...
Korsh Core uses [Doxygen](https://www.doxygen.nl/) to generate its official documentation.
...
configuration options and toggled while korshd is running with the `logging`
RPC.  For instance, launching korshd with `-debug` or `-debug=1` will turn on
...
Korsh Core is a multi-threaded application, and deadlocks or other
...
i.e. `CPPFLAGS="-DDEBUG_LOCKCONTENTION"`, then build and run korshd.

You can then use the `-debug=lock` configuration option at korshd startup or
`korsh-cli logging '["lock"]'` at runtime to turn on lock contention logging.
It can be toggled off again with `korsh-cli logging [] '["lock"]'`.
...
([`valgrind.supp`](https://github.com/SmartiesCoin/Korsh/blob/master/contrib/valgrind.supp))
...
$ valgrind -v --leak-check=full src/korshd -printtoconsole
...
To profile a running korshd process for 60 seconds, you could use an
invocation of `perf record` like this:

```sh
$ perf record \
    -g --call-graph dwarf --per-thread -F 140 \
    -p `pgrep korshd` -- sleep 60
```
...
Korsh Core can be compiled with various "sanitizers" enabled, which add
...
- [Main thread (`korshd`)](https://doxygen.bitcoincore.org/bitcoind_8cpp.html#a0ddf1224851353fc92bfbff6f499fa97)
...
However, in open source software such as Korsh Core, where everyone uses
...
pay attention to for reviewers of Korsh Core code.
...
- For `strprintf`, `LogInfo`, `LogDebug`, etc formatting characters don't need size specifiers.
  ...
  - *Rationale*: Korsh Core uses tinyformat, which is type safe. Leave them out to avoid confusion.
...
Some of these are maintained by active developers of Korsh Core, in which case
...
a Korsh Core subtree, so that they can be integrated quickly. Cosmetic changes
...
- Try not to overload methods on argument type. E.g. don't make `getblock(true)` and `getblock("hash")`
  ...
  - *Rationale*: This is impossible to use with `korsh-cli`, and can be surprising to users.
...
- Add every non-string RPC argument `(method, idx, name)` to the table `vRPCConvertParams` in `rpc/client.cpp`.
  ...
  - *Rationale*: `korsh-cli` and the GUI debug console use this table to determine how to
...
- Wallet RPCs call BlockUntilSyncedToCurrentChain to maintain consistency with
  ...
  - *Rationale*: In previous versions of Korsh Core, the wallet was always
...
- Use *invalid* Korsh addresses (e.g. in the constant array `EXAMPLE_ADDRESS`) for
