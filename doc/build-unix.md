UNIX BUILD NOTES
====================
Some notes on how to build Korsh Core in Unix.
...
memory available when compiling Korsh Core. On systems with less, gcc can be
...
To build Korsh Core without wallet, see [*Disable-wallet mode*](#disable-wallet-mode)
...
If you want to build korsh-qt, make sure that the required packages for Qt development
...
Once these are installed, they will be found by configure and a korsh-qt executable will be
built by default.
...
Note: Fedora repositories do not include libbacktrace. To build Korsh Core without stack trace support, configure with `--disable-stacktraces`.
...
...
to: /path/to/korsh/depends/x86_64-pc-linux-gnu
```
and configure using the following:
```bash
export BDB_PREFIX="/path/to/korsh/depends/x86_64-pc-linux-gnu"
...
Disable-wallet mode
--------------------
When the intention is to only run a P2P node, without a wallet, Korsh Core can
