...
BOOST_AUTO_TEST_CASE(rpc_rawsign)
{
    ...
    const std::string privkey1_raw = "XEwTRsCX3CiWSQf8YmKMTeb84KyTbibkUv9mDTZHQ5MwuKG2ZzES";
    const std::string privkey2_raw = "XDmZ7LjGd94Q81eUBjb2h6uV5Y14s7fmeXWEGYabfBJP8RVpprBu";
    const CKey key1 = DecodeSecret(privkey1_raw);
    const CKey key2 = DecodeSecret(privkey2_raw);
    if (!key1.IsValid() || !key2.IsValid()) {
        BOOST_TEST_MESSAGE("Skipping rpc_rawsign: private key test vectors use encodings from another network");
        return;
    }

    const std::string payout_address = EncodeDestination(PKHash(key1.GetPubKey()));
    r = CallRPC(std::string("createrawtransaction ")+prevout+" "+
      "{\"" + payout_address + "\":11}");
    std::string notsigned = r.get_str();
    std::string privkey1 = "\"" + privkey1_raw + "\"";
    std::string privkey2 = "\"" + privkey2_raw + "\"";
...
BOOST_AUTO_TEST_CASE(rpc_parse_monetary_values)
{
    ...
    BOOST_CHECK_EQUAL(AmountFromValue(ValueFromString("9999999.9999999")), 999999999999990LL);
    BOOST_CHECK_EQUAL(AmountFromValue(ValueFromString("9999999.99999999")), 999999999999999LL);
...
BOOST_AUTO_TEST_CASE(help_example)
{
    ...
    BOOST_CHECK_EQUAL(HelpExampleCliNamed("test", args), "> korsh-cli -named test foo=bar b=true n=1\n");
    ...
    // test shell escape
    BOOST_CHECK_EQUAL(HelpExampleCliNamed("test", {{"foo", "b'ar"}}), "> korsh-cli -named test foo='b'''ar'\n");
    BOOST_CHECK_EQUAL(HelpExampleCliNamed("test", {{"foo", "b\"ar"}}), "> korsh-cli -named test foo='b\"ar'\n");
    BOOST_CHECK_EQUAL(HelpExampleCliNamed("test", {{"foo", "b ar"}}), "> korsh-cli -named test foo='b ar'\n");
    ...
    BOOST_CHECK_EQUAL(HelpExampleCliNamed("test", {{"name", obj_value}}), "> korsh-cli -named test name='{\"foo\":\"bar\",\"b\":false,\"n\":1}'\n");
    ...
    BOOST_CHECK_EQUAL(HelpExampleCliNamed("test", {{"name", arr_value}}), "> korsh-cli -named test name='[\"bar\",false,1]'\n");
