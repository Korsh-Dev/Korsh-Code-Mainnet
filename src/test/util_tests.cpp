...
{
    const std::array<unsigned char, 32> privkey_bytes1 = {
        0xD9, 0x7F, 0x51, 0x08, 0xF1, 0x1C, 0xDA, 0x6E,
        0xEE, 0xBA, 0xAA, 0x42, 0x0F, 0xEF, 0x07, 0x26,
        0xB1, 0xF8, 0x98, 0x06, 0x0B, 0x98, 0x48, 0x9F,
        0xA3, 0x09, 0x84, 0x63, 0xC0, 0x03, 0x28, 0x66
    };
    const std::array<unsigned char, 32> privkey_bytes2 = {
        0x8F, 0xAA, 0x74, 0x15, 0x16, 0xCE, 0xCA, 0x2A,
        0x9D, 0x9E, 0xE1, 0x88, 0xFF, 0xD0, 0xA0, 0xA0,
        0x36, 0xA9, 0xCC, 0x34, 0xB7, 0xA7, 0xA6, 0x2B,
        0x4A, 0x2A, 0x4B, 0x99, 0x47, 0xD9, 0x77, 0xF1
    };

    CKey privkey1;
    privkey1.Set(privkey_bytes1.begin(), privkey_bytes1.end(), true);
    BOOST_REQUIRE(privkey1.IsValid());
    const std::string p2pkh_addr1 = EncodeDestination(PKHash(privkey1.GetPubKey()));

    CKey privkey2;
    privkey2.Set(privkey_bytes2.begin(), privkey_bytes2.end(), true);
    BOOST_REQUIRE(privkey2.IsValid());
    const std::string p2pkh_addr2 = EncodeDestination(PKHash(privkey2.GetPubKey()));

    const std::string p2sh_addr = EncodeDestination(ScriptHash(CScript() << OP_TRUE));
    ...
    BOOST_REQUIRE(MessageSign(privkey1, "Trust no one", signature1));
    ...
    BOOST_REQUIRE(MessageSign(privkey2, "Trust me", signature2));
