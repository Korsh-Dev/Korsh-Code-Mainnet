...
BOOST_AUTO_TEST_CASE(v2transport_test)
{
    ...
    // Send garbage that includes the first 15 garbage terminator bytes somewhere.
    {
        ...
        // valid Korsh IDs are [128, 128+V2_KSH_IDS.size()-1]. Generate IDs outside these ranges.
        tester.SendMessage([]() {
            if (g_insecure_rand_ctx.randbool()) {
                return static_cast<uint8_t>(InsecureRandRange(95) + 33); // Invalid Bitcoin range [33, 127]
            } else {
                return static_cast<uint8_t>(InsecureRandRange(87) + 169); // Invalid Korsh range [169, 255]
            }
        }(), {});
...
BOOST_AUTO_TEST_CASE(v2_short_id_version_negotiation)
{
    ...
    {
        V2TransportTester tester(true);
        tester.GetTransport().SetPeerVersion(BIP324_KSH_BASELINE_VERSION); // v70235
