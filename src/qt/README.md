This directory contains the source code for the Korsh Core graphical user interface (GUI). It uses the [Qt](https://www1.qt.io/developers/) cross-platform framework.
...
- Represents the main window of the Korsh UI.
...
#### paymentserver.(h/cpp)
- (Deprecated) Used to process BIP21 payment URI requests. Also handles URI-based application switching (e.g. when following a korsh:... link from a browser).

Used to process BIP21 payment URI requests. Also handles URI based application switching (e.g. when following a korsh:... link from a browser).
...
* `bitcoinunits.(h/cpp)`: KSH / mBTC / etc. handling
* `callback.h`
...
4. Enter `korsh-qt` as the Project Name and enter the absolute path to `src/qt` as Location
...
8. While in the `Projects` tab, ensure that you have the `korsh-qt` executable specified under `Run`
 - If the executable is not specified: click `"Choose..."`, navigate to `src/qt`, and select `korsh-qt`
9. You're all set! Start developing, building, and debugging the Korsh Core GUI
