...
static void SetupBitcoinUtilArgs(ArgsManager &argsman)
{
    ...
    argsman.AddCommand("hash", "Compute proof of work hash for hex header string");
...
static int AppInitUtil(ArgsManager& args, int argc, char* argv[])
{
    ...
    if (HelpRequested(args) || args.IsArgSet("-version")) {
        ...
        std::string strUsage = PACKAGE_NAME " korsh-util utility version " + FormatFullVersion() + "\n";
        if (args.IsArgSet("-version")) {
            strUsage += FormatParagraph(LicenseInfo());
        } else {
            strUsage += "\n"
                "Usage:  korsh-util [options] [commands]  Do stuff\n";
            strUsage += "\n" + args.GetHelpMessage();
        }
...
static int HashHeader(const std::vector<std::string>& args, std::string& strPrint)
{
    if (args.size() != 1) {
        strPrint = "Must specify block header to hash";
        return EXIT_FAILURE;
    }
    ...
    strPrint = header.GetHash().ToString();
    return EXIT_SUCCESS;
}
...
#endif
{
    ...
    try {
        if (cmd->command == "grind") {
            ...
            ret = HashHeader(cmd->args, strPrint);
        } else {
