# Bootstrappable Korsh Core Builds

This directory contains the files necessary to perform bootstrappable Korsh
Core builds.
...
To build Korsh Core reproducibly with all default options, invoke the
...
2. The `guix-codesign` command ***requires*** a `DETACHED_SIGS_REPO` flag.
    * _**DETACHED_SIGS_REPO**_
      ...
      Korsh Core version being built.
...
```
env DETACHED_SIGS_REPO=<path/to/korsh-detached-sigs> ./contrib/guix/guix-codesign
```
